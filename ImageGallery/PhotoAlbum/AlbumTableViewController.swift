...
class AlbumTableViewController: UITableViewController, UITableViewDragDelegate, UITableViewDropDelegate {
    ...
    func album(at indexPath: IndexPath) -> PhotoAlbum {
        return PhotoLibrary.sharedInstance.albums[indexPath.row]
    }
    
    @IBAction func addButtonItem(_ sender: UIBarButtonItem) {
        var newAlbum=PhotoAlbum()
         let existingNamesForImageGalleries: [String] = PhotoLibrary.sharedInstance.albums.map { $0.title }
        newAlbum.title = "Untitled".madeUnique(withRespectTo: existingNamesForImageGalleries)
        PhotoLibrary.sharedInstance.albums.append(newAlbum)
        let indexPath = IndexPath(row: PhotoLibrary.sharedInstance.albums.count-1, section: 0)
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return PhotoLibrary.sharedInstance.albums.count
    }
    ...
    func tableView(_ tableView: UITableView, dropSessionDidUpdate session: UIDropSession, withDestinationIndexPath destinationIndexPath: IndexPath?) -> UITableViewDropProposal {
        ...
        } else if let destinationIndexPath = destinationIndexPath, destinationIndexPath.row < PhotoLibrary.sharedInstance.albums.count {
    ...
    func tableView(_ tableView: UITableView, performDropWith coordinator: UITableViewDropCoordinator) {
        ...
        guard let destinationIndexPath = coordinator.destinationIndexPath, destinationIndexPath.row < PhotoLibrary.sharedInstance.albums.count else { return }
