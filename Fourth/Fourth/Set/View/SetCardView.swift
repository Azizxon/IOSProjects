...
class SetCardView: UIButton {
        ...
        private var drawableRect: CGRect {
            let drawableWidth = frame.size.width
            let drawableHeight = frame.size.height
    ...
    func animateFly(to center:CGPoint, delay: TimeInterval) {
        ...
        UIViewPropertyAnimator.runningPropertyAnimator(
            ...
            animations:
            {   self.isFaceUp = false
                self.center = CGPoint(x: 0, y: -center.y)
            },
            completion:
            {   position in
                UIView.transition(
